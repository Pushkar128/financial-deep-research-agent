...
# ── VERIFIED REVENUES ──────────────────────────────────────
# yfinance returns wrong/quarterly data for many Indian stocks
# These are verified annual revenues in ₹ Crores from official reports
KNOWN_REVENUES = {
    "INFY.NS":       153670,   # Infosys FY2024
    "TCS.NS":        267021,   # TCS FY2025
    "WIPRO.NS":       89818,   # Wipro FY2024
    "HCLTECH.NS":    109913,   # HCL FY2024
    "TECHM.NS":       53013,   # Tech Mahindra FY2024
    "SUNPHARMA.NS":   52041,   # Sun Pharma FY2025
    "DRREDDY.NS":     27896,   # Dr Reddy FY2024
    "CIPLA.NS":       27234,   # Cipla FY2024
    "DIVISLAB.NS":    10151,   # Divi's FY2024
    "LUPIN.NS":       19010,   # Lupin FY2024
    "AUROPHARMA.NS":  29261,   # Aurobindo FY2024
}
...
def get_stock_info(ticker: str) -> dict:
    ...
    # Use known correct revenue if available
    if ticker in KNOWN_REVENUES:
        revenue_inr_crores = KNOWN_REVENUES[ticker]
        revenue_flag = "OK"
        print(f"  ✅ Using verified revenue: ₹{revenue_inr_crores:,} Crores")
    else:
        revenue_raw = info.get("totalRevenue", "N/A")
        currency = info.get("currency", "INR")
        revenue_inr_crores = "N/A"
        revenue_flag = "MISSING"
...
def format_stock_info(info: dict) -> str:
    ...
    for key, value in info.items():
        ...
        if key == "revenue_crores":
            flag = info.get("revenue_flag", "OK")
            if flag == "UNRELIABLE":
                formatted += f"  Revenue: ₹{value} Crores ⚠️ (Verify from web)\n"
            elif flag == "MISSING":
                formatted += f"  Revenue: N/A ⚠️ (Fetch from web)\n"
            else:
